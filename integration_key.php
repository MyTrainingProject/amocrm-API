<?php

$subdomain = 'alexefilatov2012'; //Поддомен аккаунта


$client_secret = 'o9Nss16E41TwGmRVwKaKF8W2QOaGJzdUabWagLm0kQRGKayqKDK36PHBYB8prNNQ'; // Секретный ключ
$client_id     = '9da06962-f3bc-4bb1-838e-fcd043914bf6'; // ID интеграции
$code          = ''; // Код авторизации
$token_file    = 'tokens.txt';

//echo "client_secret " . $client_secret . PHP_EOL;
//echo "client_id " . $client_id . PHP_EOL;
//echo "code " . $code . PHP_EOL;
//echo "token_file " . $token_file . PHP_EOL;



